...
// GetPair returns the element for a key. If the key does not exist, the
// pointer will be nil.
func (m *OrderedMap) GetPair(key interface{}) *Pair {
	value, ok := m.kv[key]
	if ok {
