...
// MSet convert normal map[string]interface into ConcurrentMap
...
// SetIfNotExists Sets the given value under the specified key if no value was associated with it.
func (m ConcurrentMap) SetIfNotExists(key string, value interface{}) bool {
	// Get map shard.
	shard := m.GetShard(key)
	shard.Lock()
